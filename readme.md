...
| 2026-03-07 | **Globální rozdělení regionů** — Navigace nyní reálně funguje jako oddělené "stránky". Aplikace defaultně renderuje **pouze CZ**. Přepnutím na **SK** či **PL** v hlavičce dojde k překreslení veškerého obsahu i analytiky čistě jenom na konkrétní stát beze stopy případných jiných zemí. Implementována robustní detekce regionů s podporou diakritiky. |
| 2026-03-07 | **Minimalistické Excel Filtry** — Vnořené filtry (HR Konzultant, Divize, Řazení stáří) do hlaviček tabulek ve formě decentních ikon. Filtry jsou nyní izolované — v seznamu konzultantů se zobrazují pouze lidé aktivní v dané zemi. Při přepnutí země se filtry automaticky resetují pro čistý pohled. |
| 2026-03-07 | **Realistická Data & Dynamické řazení** — V `ats_sync.py` zabudována deterministická seed/hash logika pro realistická čísla (CAP 1-10 / VAC 1-2 / ACT). Přidáno cyklické řazení stáří (Vzestupně/Sestupně/Výchozí) ovládané přímo ikonou v hlavičce. |
